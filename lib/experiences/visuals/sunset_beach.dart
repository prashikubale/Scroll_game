...
class _SunsetBeachState extends State<SunsetBeach> with SingleTickerProviderStateMixin {
...
class _SunsetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Sun Glow
    canvas.drawCircle(sunCenter, sunRadius * 1.2, Paint()..color = Colors.orangeAccent.withValues(alpha: 0.2)..maskFilter = MaskFilter.blur(BlurStyle.normal, 40));
    ...
    final stripePaint = Paint()..color = Color(0xFF8D2564).withValues(alpha: 0.5)..style = PaintingStyle.fill;
    for (int i = 0; i < 6; i++) {
        double y = sunCenter.dy + (sunRadius * 0.3) + (i * sunRadius * 0.15);
        double height = sunRadius * 0.05 + (i * 2);
        if (y < sunCenter.dy + sunRadius) {
             canvas.drawRect(Rect.fromLTWH(sunCenter.dx - sunRadius, y, sunRadius * 2, height), stripePaint);
        }
    }
    ...
    // Grid / Waves on Water
    final gridPaint = Paint()..color = Colors.cyanAccent.withValues(alpha: 0.3)..strokeWidth = 1;
    final perspectiveOrigin = Offset(w / 2, h * 0.6); // Horizon center
    
    // Vertical Perspective Lines
    for (double i = -1; i <= 2; i+=0.2) {
         double xBase = w * i;
         canvas.drawLine(perspectiveOrigin, Offset(xBase - (w/2 - perspectiveOrigin.dx) * 4, h), gridPaint);
    }
    ...
    for (int i = 0; i < 20; i++) {
        ...
        canvas.drawLine(Offset(0, y), Offset(w, y), gridPaint..color = Colors.cyanAccent.withValues(alpha: 0.1 + (p * 0.4)));
    }
    
    // Reflection of Sun
    final reflectionPaint = Paint()..shader = LinearGradient(colors: [Colors.orange.withValues(alpha: 0.6), Colors.transparent], begin: Alignment.topCenter, end: Alignment.bottomCenter).createShader(Rect.fromLTWH(0, h*0.6, w, h*0.4));
    Path path = Path();
