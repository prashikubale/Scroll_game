...
class SurfaceParadoxGameController extends ChangeNotifier implements MiniGame {
  ...
  void move(double deltaX, double deltaY) {
    ...
    if (_gravityDir.dy.abs() > 0.5) {
      // Gravity is Vertical (Down or Up) -> Use X input
      _playerVel = Offset(deltaX * moveSpeed, _playerVel.dy);
    } else {
      // Gravity is Horizontal (Left or Right) -> Use Y input
  ...
  void _checkWallCollision() {
    ...
    if (hit) {
      _isGrounded = true; 
      // Stop velocity in the gravity direction
      if (_gravityDir.dx.abs() > 0) {
        _playerVel = Offset(0, _playerVel.dy * 0.9);
      } else {
...
class SurfaceParadoxGameWidget extends StatelessWidget {
  final SurfaceParadoxGameController controller;

  const SurfaceParadoxGameWidget({super.key, required this.controller});
                   ...
                   Center(
                     child: Column(
                       ...
                       children: [
                         ...
                         Text(
                           "Tap to Jump • Swipe to Move\nGravity changes to the wall you touch.",
